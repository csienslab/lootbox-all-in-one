...
class TrustedSetup(NamedTuple):
    ...
    def generate(cls, F: GF, t: int, g1andg2: bool):
        ...
        # alpha = F.random()
        alpha = F(
            2297563003219932954408309842445761013569773548462794836558733181106172637922
        )
        alpha_powers = [F(1)]
        g1_powers = [curve.G1]
        g2_powers = [curve.G2]
...
def CommitSumTrusted(PK: TrustedSetup, coeff: List[Field]):
    return reduce(
        operator.add, [PK.alpha_powers[i] * c_i for i, c_i in enumerate(coeff)]
    )
...
def CommitSum(PK: TrustedSetup, coeff: List[Field]):
    ...
    return reduce(
        curve.add,
        [curve.multiply(PK.g1_powers[i], int(c_i)) for i, c_i in enumerate(coeff)],
    )
